function [FSEOFRxnList, KOtable] = FSEOFKnock(model,RxnName,biomassRxn)
...
FSEOFRxnList = setdiff(decflux_rxns,transRxns);
...
if length(FSEOFRxnList) <= threshold
    C = nchoosek(FSEOFRxnList,2);  %all possible combinations of 2 rxn deletions
    for q = 1:length(C)
        modelnew1 = model;
        ...
        if fbaMT1.stat ==1
            ...
            if ~isempty(productFluxMT1{q,1}) && ~isempty(growthOrg1_MT1{q,1}) && ~isempty(growthOrg2_MT1{q,1}) && ~isempty(productFluxWT)
                if (productFluxMT1{q,1} ~= 0 && round(productFluxMT1{q,1}, 4) > round(productFluxWT, 4)) && (growthOrg1_MT1{q,1} >= 0.01) && (growthOrg2_MT1{q,1} >= 0.01)
                    P1 = C(q,1);
        ...
        if fbaMT2.stat ==1
            ...
            if ~isempty(productFluxMT2{q,1}) && ~isempty(growthOrg1_MT2{q,1}) && ~isempty(growthOrg2_MT2{q,1}) && ~isempty(productFluxWT)
        ...
        if fbaMT3.stat ==1
            ...
            if ~isempty(productFluxMT3{q,1}) && ~isempty(growthOrg1_MT3{q,1}) && ~isempty(growthOrg2_MT3{q,1}) && ~isempty(productFluxWT)
    ...
    for q = 1:length(FSEOFRxnList) %perform single rxn deletions only
        modelnew = model;
        rxnRemoveList = FSEOFRxnList(q);
